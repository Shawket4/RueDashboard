...
export default function Inventory() {
  ...
  const { data: items = [], isLoading: stockLoading } = useQuery({
    ...
    queryFn: () =>
      inventoryApi.getInventoryItems(activeBranch!.id).then((r) => r.data),
  ...
  const { data: adjustments = [], isLoading: adjLoading } = useQuery({
    ...
    queryFn: () =>
      inventoryApi.getAdjustments(activeBranch!.id).then((r) => r.data),
    enabled: !!activeBranch?.id,
  });
  ...
  const { data: transfers = [], isLoading: transLoading } = useQuery({
    ...
    queryFn: () =>
      inventoryApi.getTransfers(activeBranch!.id).then((r) => r.data),
    enabled: !!activeBranch?.id,
  });
    ...
    mutationFn: () => {
      const payload = {
        ...
        cost_per_unit: itemForm.cost_per_unit
          ? parseFloat(itemForm.cost_per_unit)
          : null,
  ...
  const adjMutation = useMutation({
    mutationFn: () =>
      inventoryApi.createAdjustment(activeBranch!.id, {
  ...
  const stockCols: ColumnDef<InventoryItem, any>[] = [
    ...
    },
    {
      ...
      header: "Unit",
      cell: ({ row }) => (
        <Badge variant="outline">{fmtUnit(row.original.unit)}</Badge>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        const pct = Math.min(
          ...
          (row.original.current_stock /
            Math.max(row.original.reorder_threshold * 2, 1)) *
            100,
        );
        const low =
          row.original.current_stock <= row.original.reorder_threshold;
        return (
          <div className="flex items-center gap-2 min-w-[120px]">
            <Progress
              value={pct}
              className={`h-1.5 flex-1 ${low ? "[&>div]:bg-amber-500" : ""}`}
            />
            <span
              className={`text-xs tabular-nums font-semibold ${low ? "text-amber-600" : ""}`}
            >
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-xs text-muted-foreground">
          {row.original.reorder_threshold} {fmtUnit(row.original.unit)}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) =>
        row.original.cost_per_unit ? (
          <span className="text-xs tabular-nums">
            {egp(row.original.cost_per_unit * 100)}
          </span>
        ) : (
  ...
  const adjCols: ColumnDef<InventoryAdjustment, any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge
          variant={
            row.original.adjustment_type === "add" ? "success" : "warning"
          }
        >
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="tabular-nums text-sm">
          {row.original.quantity} {fmtUnit(row.original.unit)}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "By",
      cell: ({ row }) => (
        <span className="text-xs">{row.original.adjusted_by_name}</span>
      ),
    },
    {
  ...
  const transCols: ColumnDef<InventoryTransfer, any>[] = [
    ...
    },
    {
      ...
      header: "From",
      cell: ({ row }) => (
        <span className="text-xs">{row.original.source_branch_name}</span>
      ),
    },
    {
      ...
      header: "To",
      cell: ({ row }) => (
        <span className="text-xs">{row.original.destination_branch_name}</span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <span className="tabular-nums text-sm">
          {row.original.quantity_sent} {fmtUnit(row.original.unit)}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-xs text-muted-foreground">
          {fmtDateTime(row.original.initiated_at)}
        </span>
      ),
    },
  ];
    ...
    return (
      <div className="p-6 lg:p-8">
        <EmptyState
          ...
          sub="Select a branch from the sidebar to view inventory"
        />
      </div>
    );
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="mb-6">
          <TabsTrigger value="stock">
            Stock ({items.length})
            {lowStock.length > 0 && (
              <Badge variant="warning" className="ml-1 h-4 text-[10px]">
                {lowStock.length}
              </Badge>
            )}
          </TabsTrigger>
        ...
        <DialogContent>
          ...
          <div className="p-6 space-y-4">
            <div className="space-y-1.5">
              <Label>Item</Label>
              <Select
                value={adjForm.inventory_item_id}
                onValueChange={(v) =>
                  setAdjForm((f) => ({ ...f, inventory_item_id: v }))
                }
              >
                <SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setAdjForm((f) => ({ ...f, adjustment_type: v }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAdjDialog(false)}>
              Cancel
            </Button>
            <Button
        ...
        <DialogContent>
          ...
          <div className="p-6 space-y-4">
            <div className="space-y-1.5">
              <Label>Item</Label>
              <Select
                value={transForm.inventory_item_id}
                onValueChange={(v) =>
                  setTransForm((f) => ({ ...f, inventory_item_id: v }))
                }
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select item…" />
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setTransForm((f) => ({ ...f, destination_branch_id: v }))
                }
              >
                <SelectTrigger>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                type="number"
                step="0.1"
                value={transForm.quantity_sent}
                onChange={(e) =>
                  setTransForm((f) => ({ ...f, quantity_sent: e.target.value }))
                }
                placeholder="0"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setTransForm((f) => ({ ...f, note: e.target.value }))
                }
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setTransDialog(false)}>
              Cancel
            </Button>
            <Button
